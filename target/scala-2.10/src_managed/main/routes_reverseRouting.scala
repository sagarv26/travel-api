...
// @LINE:11
class ReverseGetValue {
    

// @LINE:11
def getAllDist(): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "districts")
}
...
def getAllDist : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.GetValue.getAllDist",
   """
      function() {
...
def getAllDist(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.GetValue]).getAllDist(), HandlerDef(this, "controllers.GetValue", "getAllDist", Seq(), "GET", """""", _prefix + """districts""")
)
